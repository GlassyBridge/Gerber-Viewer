...
export function getLayers(layerData) {
    const layers = {};

    for (const layerID in layerData) {
        const layerDatum = layerData[layerID];
        const commands = layerDatum.children;
        ...
        for (const toolCode in toolDefinitions) {
            ...
            if (tool.type === 'macroShape'){
                ...
                const toolParams = tool.variableValues; // Holds parameters.
                ...
                const resolvedPrimitives = macroPrimitives.map(macroPrimitive => {
                    ...
                    try {
                        resolvedModifiers = macroPrimitive.parameters.map(modifier => {
